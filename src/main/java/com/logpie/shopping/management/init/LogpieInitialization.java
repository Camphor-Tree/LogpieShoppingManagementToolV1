...
public class LogpieInitialization
{
    ...
    private void initDatabase()
    {
        ...
        jdbcTemplate
                .execute("create table if not exists Categories(CategoryId serial primary key, CategoryName text not null, IsActivated boolean not null default true)");
        jdbcTemplate
                .execute("create table if not exists Brands(BrandId serial primary key, BrandEnglishName text not null, BrandChineseName text, IsActivated boolean not null default true, BrandImageId bigint unsigned not null, BrandSizeChartId bigint unsigned, CategoryId bigint unsigned not null, foreign key (BrandImageId) references Images(ImageId) on update cascade on delete cascade, foreign key (BrandSizeChartId) references Images(ImageId) on update cascade on delete cascade, foreign key (CategoryId) references Categories(CategoryId) on update cascade on delete cascade)");
        jdbcTemplate
                .execute("create table if not exists Products(ProductId serial primary key, ProductName text not null, ProductDescription text, ProductLink text, ProductWeight int unsigned, IsActivated boolean not null default true, PostDate timestamp not null default CURRENT_TIMESTAMP, ProductImageId bigint unsigned, BrandId bigint unsigned, foreign key (BrandId) references Brands(BrandId) on update cascade on delete cascade, foreign key (ProductImageId) references Images(ImageId) on update cascade on delete cascade)");
        // jdbcTemplate
        ...
        jdbcTemplate
                .execute("create table if not exists Orders(OrderId serial primary key, OrderDate timestamp not null default current_timestamp, ProductId bigint unsigned not null, ProductCount int not null, OrderBuyerName text not null, OrderProxy bigint unsigned, OrderProxyProfitPercentage float default 0.4 not null, CurrencyRate float default 6.23 not null, PackageId bigint unsigned, EstimatedShippingFee bigint not null,ActualShippingFee bigint, SellingPrice bigint not null, CustomerPaidMoney bigint not null,FinalProfit bigint, IsProfitPaid boolean not null default false,OrderNote text, foreign key (OrderProxy) references Admins(AdminId) on update cascade on delete cascade)");
        // jdbcTemplate
