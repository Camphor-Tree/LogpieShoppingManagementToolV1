...
public class LogpieInitialization
{
    ...
    private void initDatabase()
    {
        ...
        jdbcTemplate
                .execute("create table if not exists Packages(PackageId serial primary key, TrackingNumber text not null, PackageReceiver text not null, PackageDestination text not null, PackageDate datetime not null, PackageWeight int not null, ShippingCategory char(1) not null, ShippingFee float not null, CustomTaxFee float, InsuranceFee float, IsShipped boolean not null default false, IsArrived boolean not null default false, PackageNote text)");
        jdbcTemplate
